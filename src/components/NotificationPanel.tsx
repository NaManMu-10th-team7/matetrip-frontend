...
const getNotificationTitle = (content: string): string => {
  ...
  if (content.includes('여행 일정이 업데이트되었습니다')) return '일정 업데이트';
  ...
  useEffect(() => {
    ...
    eventSource.addEventListener('new_notification', (event) => {
      try {
        ...
        toast.info(
          <div onClick={() => toast.dismiss(uniqueToastId)} style={{ cursor: 'pointer' }}>
    ...
    eventSource.addEventListener('unread-update', (event) => {
      try {
        ...
        console.error('[SSE] 읽지 않은 알림 개수 업데이트 중 오류 발생:', error);
      }
    });
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <h4 className="text-gray-900 text-sm font-semibold">{getNotificationTitle(notification.content)}</h4>
                  ...
                  <p className="text-gray-400 text-xs mt-2">{formatTimeAgo(notification.createdAt)}</p>
                </div>
              </div>
            </button>
          ))}
